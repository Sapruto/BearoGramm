...
class WebSocketStateRepository(BaseRedisRepository[WebSocketStateMapper, WebSocketStateFields, WebSocketStateEntity]):
    ...
    async def publish_notification(self, user_uuid: str, notification: Dict[str, Any]) -> None:
        channel = f"{self.USER_NOTIFICATIONS_PREFIX}:{user_uuid}"
        await self.redis.publish(channel, json.dumps(notification))

    async def publish_to_chat(self, chat_uuid: str, notification: Dict[str, Any]) -> None:
...
def get_websocket_state_repository(redis: Optional[Redis] = None) -> WebSocketStateRepository:
