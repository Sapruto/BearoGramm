...
async def get_login_token(request: SendCodeRequest, service: UserService = Depends(get_user_service)):
...
async def verify_phone(request: VerifyCodeRequest, service: UserService = Depends(get_user_service)):
