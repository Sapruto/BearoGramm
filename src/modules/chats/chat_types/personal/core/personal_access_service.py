...
from ..core.personal_repository import (
    PersonalChatRepository,
    get_personal_chat_repository,
)
...
class PersonalAccessService:
    ...
    def _get_user_access(
        self, chat: ChatEntity, user_uuid: str
    ) -> Optional[PersonalAccessType]:
        ...
        try:
            if user_uuid == companion_uuid:
                return CreateChatResponse(
                    success=False, error_message="Нельзя создать чат с самим собой"
                )
            ...
            return CreateChatResponse(
                success=False, error_message=f"Ошибка создания чата: {str(e)}"
            )
        ...
        try:
            ...
            if not chat:
                return GetChatResponse(
                    success=False, error_message=f"Чат {chat_uuid} не найден"
                )

            if not self._is_user_in_chat(chat, user_uuid):
                return GetChatResponse(success=False, error_message="Вы не в этом чате")

            if not self._validate_personal_chat(chat):
                return GetChatResponse(
                    success=False, error_message="Чат не является личным"
                )
            ...
            return GetChatResponse(
                success=False, error_message=f"Ошибка получения чата: {str(e)}"
            )
    ...
    async def get_chats_for_user(
        ...
        try:
            ...
            return ListChatsResponse(success=True, chats=chats, total=total)

        except Exception as e:
            return ListChatsResponse(
                success=False, error_message=f"Ошибка получения чатов: {str(e)}"
            )
    ...
    async def get_contact_list(
        ...
        try:
            contacts, total = await self.repo.get_contacts_with_status(
                user_uuid, limit, offset
            )
            return ContactsResponse(success=True, contacts=contacts, total=total)

        except Exception as e:
            return ContactsResponse(
                success=False, error_message=f"Ошибка получения контактов: {str(e)}"
            )

    async def find_chat_between_users(
        self, user_uuid: str, companion_uuid: str
    ) -> FindChatResponse:
        try:
            ...
            return FindChatResponse(chat=None, found=False)

    async def block_user_in_chat(
        self, chat_uuid: str, blocker_uuid: str, blocked_uuid: str
    ) -> BlockUserResponse:
        try:
            chat = await self.repo.get_by_uuid(chat_uuid)
            if not chat:
            ...
            if not self._is_user_in_chat(chat, blocker_uuid):
                return BlockUserResponse(
                    success=False, error_message="Вы не в этом чате"
                )
            ...
            if blocker_uuid == blocked_uuid:
                return BlockUserResponse(
                    success=False, error_message="Нельзя заблокировать самого себя"
                )

            if blocked_access.is_blocked:
                return BlockUserResponse(
                    success=False, error_message="Пользователь уже заблокирован"
                )
            ...
            return BlockUserResponse(
                success=False, error_message=f"Ошибка блокировки: {str(e)}"
            )

    async def unblock_user_in_chat(
        self, chat_uuid: str, user_uuid: str, unblocker_uuid: str
    ) -> UnblockUserResponse:
        try:
            chat = await self.repo.get_by_uuid(chat_uuid)
            if not chat:
            ...
            if not user_access:
                return UnblockUserResponse(
                    success=False, error_message=f"Пользователь {user_uuid} не в чате"
                )

            if not user_access.is_blocked:
                return UnblockUserResponse(
                    success=False, error_message="Пользователь не заблокирован"
                )
            ...
            return UnblockUserResponse(
                success=False, error_message=f"Ошибка разблокировки: {str(e)}"
            )
    ...
    async def delete_chat(self, chat_uuid: str, user_uuid: str) -> DeleteChatResponse:
        try:
            ...
            return DeleteChatResponse(
                success=False, error_message=f"Ошибка удаления чата: {str(e)}"
            )
