...
from ..core.personal_access_service import (
...
@personal_chats_router.post(
    PersonalChatsRoutes.create, response_model=CreateChatResponse
)
async def create_personal_chat(
    current_user: UserEntity = Depends(get_current_user_depends()),
    companion_uuid: str = Query(...),
    service: PersonalAccessService = Depends(get_personal_access_service),
):
    return await service.create_personal_chat(current_user.uuid, companion_uuid)


@personal_chats_router.get(PersonalChatsRoutes.get, response_model=GetChatResponse)
...
@personal_chats_router.get(PersonalChatsRoutes.list, response_model=ListChatsResponse)
...
@personal_chats_router.get(
    PersonalChatsRoutes.contacts, response_model=ContactsResponse
)
...
@personal_chats_router.get(PersonalChatsRoutes.find, response_model=FindChatResponse)
...
@personal_chats_router.post(PersonalChatsRoutes.block, response_model=BlockUserResponse)
async def block_user_in_chat(
    current_user: UserEntity = Depends(get_current_user_depends()),
    chat_uuid: str = Path(...),
    blocked_uuid: str = Query(...),
    service: PersonalAccessService = Depends(get_personal_access_service),
):
...
@personal_chats_router.post(
    PersonalChatsRoutes.unblock, response_model=UnblockUserResponse
)
...
@personal_chats_router.delete(
    PersonalChatsRoutes.delete, response_model=DeleteChatResponse
)
