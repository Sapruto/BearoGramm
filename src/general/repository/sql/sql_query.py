...
class SqlQuery(QueryInterface):
    ...
    limit: Optional[int] = Field(None, ge=1, description="Maximum number of records to return")
    offset: Optional[int] = Field(None, ge=0, description="Number of records to skip")
    order_by: Optional[List[Tuple[Fields, str]]] = Field(None, description="List of (field, direction) tuples for sorting")
    ...
    @classmethod
    def validate_filters(cls, v: Optional[Dict[Fields, Any]]) -> Optional[Dict[Fields, Any]]:
