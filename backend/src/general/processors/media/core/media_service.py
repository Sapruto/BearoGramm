...
class MediaProcessor(BaseDataProcessor[MediaData]):
    def __init__(
        self,
    ...
    async def save_data(self, raw_data: Any) -> MediaData:
        try:
            if isinstance(raw_data, dict):
                content = raw_data.get("content")
                filename = raw_data.get("filename", "file.bin")
                prefix_uuid = raw_data.get("prefix_uuid")
            ...
            file_path = self.media_utils.generate_path(filename, prefix_uuid)
            ...
            return MediaData(data_type=self.data_type, media_url=result)

        except Exception as e:
            logger.error(f"Media process error: {e}")
            raise

    async def delete_data(self, processed_data: MediaData) -> bool:
        try:
    ...
    async def prepare_to_save(self, data: MediaData) -> MediaData:
        return data

    async def prepare_to_use(self, data: MediaData) -> MediaData:
        return data


def get_media_message_service() -> MediaProcessor:
