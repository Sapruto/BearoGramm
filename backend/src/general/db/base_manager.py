...
class BaseManager(Generic[ORM], ABC):
    ...
    async def get_by_field(
        ...
        select_field: Optional[InstrumentedAttribute] = None,
    ...
    async def count(
        self,
        where: Optional[Dict[InstrumentedAttribute, Any]] = None,
        session: Optional[AsyncSession] = None,
    ) -> int:
        async with self.__get_session(session) as sess:
    ...
    async def create(
        self,
        model: ORM,
        on_conflict: OnConflictAction = OnConflictAction.NOTHING,
        commit: bool = True,
        session: Optional[AsyncSession] = None,
    ) -> Optional[ORM]:
        if not model:
        ...
        if not session and not commit:
            raise InvalidTransactionStateError(
                "A commit cannot be false if no session is passed"
            )
        if session and not commit:
            logger.warning(
                "Session provided with commit=False - caller must manage transaction"
            )
            ...
            if on_conflict == OnConflictAction.UPDATE:
                ...
                existing = await sess.execute(
                    select(self.model).where(
                        ...
                        == getattr(model, self.identifier_field.key)
                    )
                )
    ...
    async def update(
        ...
        field_identifier: Optional[InstrumentedAttribute] = None,
        ...
        **kwargs,
        ...
        if where is None and (not identifier or not field_identifier):
            raise ValueError(
                "Either 'where' or ('identifier' and 'field_identifier') must be provided"
            )
        if where is not None and identifier and field_identifier:
            raise ValueError(
                "Cannot use both 'where' and ('identifier' + 'field_identifier') at the same time"
            )
    ...
    async def exists(
        self,
        identifier: Any,
        field_identifier: InstrumentedAttribute,
        session: Optional[AsyncSession] = None,
    ) -> bool:
        async with self.__get_session(session) as sess:
