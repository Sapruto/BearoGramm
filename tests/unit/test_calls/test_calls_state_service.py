...
class TestCallsStateService:
    ...
    async def test_update_call_status(
        self, calls_state_service, sample_call_state_entity
    ):
        calls_state_service.calls_state_repository.save = AsyncMock()

        await calls_state_service._update_call_status(
            sample_call_state_entity, "answered"
        )
    ...
    @pytest.mark.asyncio
    async def test_update_call_status_rejected(
        self, calls_state_service, sample_call_state_entity
    ):
        calls_state_service.calls_state_repository.save = AsyncMock()

        await calls_state_service._update_call_status(
            sample_call_state_entity, "rejected"
        )

        assert sample_call_state_entity.status == CallStatus.REJECTED

    @pytest.mark.asyncio
    async def test_update_call_status_timeout(
        self, calls_state_service, sample_call_state_entity
    ):
        calls_state_service.calls_state_repository.save = AsyncMock()

        await calls_state_service._update_call_status(
            sample_call_state_entity, "timeout"
        )

        assert sample_call_state_entity.status == CallStatus.TIMEOUT

    @pytest.mark.asyncio
    async def test_update_call_status_unknown(
        self, calls_state_service, sample_call_state_entity
    ):
        calls_state_service.calls_state_repository.save = AsyncMock()

        await calls_state_service._update_call_status(
            sample_call_state_entity, "unknown"
        )
    ...
    @pytest.mark.asyncio
    async def test_send_pending_calls_with_entities(
        self, calls_state_service, sample_call_state_entity
    ):
        mock_ws_send = AsyncMock()
        calls_state_service.calls_state_repository.get_all = AsyncMock(
            return_value=[sample_call_state_entity]
        )
    ...
    async def test_handle_accept(self, calls_state_service):
        ...
        data = {"caller_uuid": caller_uuid, "sdp_answer": "test_answer"}
        mock_ws_send = AsyncMock()

        calls_state_service.calls_state_repository.notify_user = AsyncMock()
        calls_state_service.calls_state_repository.get_by_id = AsyncMock(
            return_value=None
        )
        calls_state_service.calls_state_repository.save = AsyncMock()
    ...
    @pytest.mark.asyncio
    async def test_handle_accept_with_room(
        self, calls_state_service, sample_room_call_entity
    ):
        user_uuid = str(uuid4())
        caller_uuid = str(uuid4())
        room_id = str(uuid4())
        ...
        calls_state_service.calls_state_repository.notify_user = AsyncMock()
        calls_state_service.calls_state_repository.get_by_id = AsyncMock(
            return_value=sample_room_call_entity
        )
        calls_state_service.calls_state_repository.save = AsyncMock()
    ...
    async def test_handle_reject(self, calls_state_service):
        ...
        calls_state_service.calls_state_repository.notify_user = AsyncMock()
        calls_state_service.calls_state_repository.delete_by_id = AsyncMock(
            return_value=True
        )
        ...
        calls_state_service.calls_state_repository.notify_user.assert_called_once()
        calls_state_service.calls_state_repository.delete_by_id.assert_called_once_with(
            caller_uuid
        )
    ...
    async def test_handle_ice(self, calls_state_service):
        ...
        data = {"target_uuid": target_uuid, "candidate": "test_candidate"}
        mock_ws_send = AsyncMock()
        ...
        calls_state_service.calls_state_repository.notify_user.assert_called_once_with(
            target_uuid, {"type": "ice_candidate", "candidate": "test_candidate"}
        )
    ...
    async def test_handle_command(self, calls_state_service):
        ...
        await calls_state_service._handle_command(
            user_uuid, {"type": "accept_call"}, mock_ws_send
        )
        calls_state_service._handle_accept.assert_called_once()

        await calls_state_service._handle_command(
            user_uuid, {"type": "reject_call"}, mock_ws_send
        )
        calls_state_service._handle_reject.assert_called_once()

        await calls_state_service._handle_command(
            user_uuid, {"type": "ice_candidate"}, mock_ws_send
        )
        calls_state_service._handle_ice.assert_called_once()

        await calls_state_service._handle_command(
            user_uuid, {"type": "hangup"}, mock_ws_send
        )
    ...
    async def test_handle_command_ping(self, calls_state_service):
        ...
        await calls_state_service._handle_command(
            user_uuid, {"type": "ping"}, mock_ws_send
        )
    ...
    async def test_handle_command_unknown(self, calls_state_service):
        ...
        await calls_state_service._handle_command(
            user_uuid, {"type": "unknown"}, mock_ws_send
        )
    ...
    async def test_call_success(self, calls_state_service):
        ...
        calls_state_service.calls_state_repository.save = AsyncMock()
        calls_state_service.calls_state_repository.notify_user = AsyncMock()
        calls_state_service.push_client_api.send_call_push = AsyncMock(
            return_value=True
        )
    ...
    async def test_call_missing_callee(self, calls_state_service):
        ...
        mock_ws_receive = AsyncMock()
        mock_ws_receive.return_value = json.dumps(
            {"type": "call_offer", "sdp_offer": "test_offer"}
        )

        await calls_state_service.call(user_uuid, mock_ws_send, mock_ws_receive)

        mock_ws_send.assert_called_with(
            json.dumps({"error": "Missing callee_uuid or sdp_offer"})
        )
    ...
    async def test_call_timeout(self, calls_state_service):
        ...
        calls_state_service._wait_for_answer = AsyncMock(
    ...
    def test_get_calls_state_service(self):
        from src.modules.calls.core.services.calls_state_service import (
            get_calls_state_service,
        )
